...
bool ColorShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	WCHAR pixelshader[] = L"../RenderWithDirectX/colorpixel.hlsl";

	WCHAR vertexshader[] = L"../RenderWithDirectX/colorvertex.hlsl";
...
bool ColorShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	// Initialize the pointers this funciton will use to null.
	errorMessage = 0;
	...
	if (FAILED(result))
	{
		// If the shader failed to compile it should havbe written something to the error message.
		if (errorMessage)
		{
			MessageBox(hwnd, L"fucking vertex", L"Error", MB_OK);
	...
	if (FAILED(result))
	{
		// If the shader failed to compile it should have writtem something to the error message.
		if (errorMessage)
		{
		...
		MessageBox(hwnd, L"fucking pixel", L"Error", MB_OK);
	...
	if (FAILED(result))
	{
		MessageBox(hwnd, L"fucking create vertex shader", L"Error", MB_OK);
		return false;
	}
	...
	if (FAILED(result))
	{
		MessageBox(hwnd, L"fucking create pixel shader", L"Error", MB_OK);
		return false;
	}
